...
class CheckIP(SimpleTask):
    ...
    def process(self, item):
        ...
        if self._counter <= 0:
            ...
            response = requests.get('https://cdn2-s-hw-e5.xtube.com/m=eoeM8f/videos/202010/09/45185361/xtube_original/4.jpg', allow_redirects=False)
            assert response.status_code == 200
            assert len(response.content) == 15083

            response = requests.get('https://www.xtube.com/', allow_redirects=False)
            assert response.status_code == 200
            assert b'/video/gay-group-sex/mrc' in response.content
            assert b'Gay Group Sex' in response.content
...
LUA_SHA1 = get_hash(os.path.join(CWD, 'xtube.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'v':
                wget_args.extend(['--warc-header', 'video: '+item_value])
                wget_args.append('https://www.xtube.com/video-watch/-'+item_value)
            elif item_type == 'p':
                wget_args.extend(['--warc-header', 'profile: '+item_value])
                wget_args.append('https://www.xtube.com/profile/--'+item_value)
            elif item_type == 's':
                wget_args.extend(['--warc-header', 'search: '+item_value])
                wget_args.append('https://www.xtube.com/search/video/'+item_value)
            elif item_type == 'cdn':
                wget_args.append('https://cdn'+item_value)
            else:
...
project = Project(
    title = 'XTube',
    project_html = '''
    <img class="project-logo" alt="logo" src="https://wiki.archiveteam.org/images/d/de/Xtube_logo_black.png" height="50px"/>
    <h2>xtube.com <span class="links"><a href="https://xtube.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/xtube/">Leaderboard</a></span></h2>
    '''
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://{}/{}/multi={}/'
        ...
        downloader, VERSION),
    PrepareDirectories(warc_prefix='xtube'),
    WgetDownload(
        WgetArgs(),
        max_tries=1,
